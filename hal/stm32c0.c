...
#define RCC_IOPENR          (*(volatile uint32_t *)(RCC_BASE + 0x34))  /* RM0490 - 5.4.12 - IO port enable register */
#define APB1_CLOCK_ER       (*(volatile uint32_t *)(RCC_BASE + 0x3C))  /* RM0490 - 5.4.13 - APBENR1 */
#define APB2_CLOCK_ER       (*(volatile uint32_t *)(RCC_BASE + 0x40))  /* RM0490 - 5.4.14 - APBENR2 */
...
#define UART2_PIN_AF           1  /* AF1 for UART2 on C0 */
 ...
 * - After reset: HSIDIV = 010 (divide by 4), so HSI = 48/4 = 12 MHz
 * - UART2 uses PCLK1 which equals SYSCLK when APB prescaler = 1
 * - BRR = PCLK1 / bitrate (for 16x oversampling, handled by hardware)
 */
#ifndef PCLK1_FREQ
#define PCLK1_FREQ            (12000000)  /* 12 MHz (HSI/4 after reset) */
#endif
#ifndef CLOCK_SPEED
