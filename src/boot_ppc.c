 ...
 * Sets up 36-bit physical addressing per FUM Table 2.5 (64-bit Memory Map).
 * Must run from RAMFUNCTION (DDR) since we modify flash/CCSR LAW and TLB.
 *
 * Key changes from 32-bit intermediary map (Table 2.6):
 * - EA 0x80000000 → PA 0xC_00000000: DDR upper 2GB (was PCIe in intermediary)
 * - All peripheral LAWs use BARH=0xF (36-bit physical addressing)
 * - TLB MAS7 matches LAW BARH for flash and CCSR */
static void RAMFUNCTION hal_os64bit_map_transition(void)
{
    ...
    /* Note: DDR LAW is now 4GB at PA 0x0 (set in hal_ddr_init).
     * No separate upper DDR LAW needed — the 4GB LAW covers all DDR.
     *
     * PCIe LAWs match CW VPX3-152 FUM Table 2.5 (ostype2 final 64-bit
     * map, used by VxWorks 7 64-bit). Production CW U-Boot's
     * cw_late_mmap_adjust() converts the intermediary 32-bit map
     * (Table 2.6, 512MB/1GB PCIe windows at low PA upper) to this
     * final layout when bootm runs. wolfBoot needs the same final
     * layout at handoff. */
    ...
    /* TLB entries matching CW U-Boot tlb_table_lnx_vx7 (tlb.c).
     * CCSR (Entry 1) and Flash (Entry 2) are already set from boot
     * assembly with correct MAS7 = PHYS_HIGH. Add the remaining
     * peripheral TLBs that VxWorks needs during early init. */

    /* TLB entries for peripheral access by the OS.
     * IMPORTANT: Avoid ESELs used by wolfBoot:
     *   0 = boot ROM (invalidated on VPX3), 1 = CCSR, 2 = flash,
     *   9 = .ramcode (CPC SRAM / DDR), 12 = DDR lower 2GB.
     * Use ESELs 3-8, 10-11, 13+ for new entries. */
    ...
    set_tlb(1, 6, 0xEE400000, 0xEE400000, 0xF,
        MAS3_SX | MAS3_SW | MAS3_SR, MAS2_I | MAS2_G, 0,
        BOOKE_PAGESZ_4M, 1);
    /* PCIe4 (Switch) memory: EA 0xC0000000 → PA 0xD_00000000, 2GB
     * (Table 2.5 final ostype2 64-bit map, post-cw_late_mmap_adjust).
     * Was previously labelled "PCIe1 EA=0xC0000000" -- but per the
     * CW FUM, EA=0xC0000000 is PCIe4 (Switch), not PCIe1. */
    set_tlb(1, 7, 0xC0000000, 0x00000000, 0xD,
        MAS3_SX | MAS3_SW | MAS3_SR, MAS2_I | MAS2_G, 0,
        BOOKE_PAGESZ_2G, 1);
    /* PCIe1 (XMC) memory: EA 0xE0000000 → PA 0xF_E0000000, 256MB.
     * Table 2.5 lists 160MB; round up to 256MB for the power-of-two
     * page-size encoding. */
    set_tlb(1, 8, 0xE0000000, 0xE0000000, 0xF,
        MAS3_SX | MAS3_SW | MAS3_SR, MAS2_I | MAS2_G, 0,
        BOOKE_PAGESZ_256M, 1);
    /* Note: EA 0x80000000 -> PA 0xC_00000000 is RAM space (DDR upper 2GB)
     * in the FUM Table 2.5 final 64-bit map; VxWorks creates that mapping
     * itself, so we don't add it here. */
