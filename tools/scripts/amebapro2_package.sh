...
# unintended directory. Reject non-absolute first, then canonicalize (readlink
# -m collapses '.'/'..' and needs no existing path) so a value that normalizes
# to a dangerous location -- e.g. "/tmp/../" resolving to "/" -- is caught too.
# The default (/tmp/wolfboot_amebapro2_pkg) is safe.
case "$OUTDIR" in
    /*) ;;
...
echo "built: $OUTDIR/flash_ntz.bin ($(stat -c %s flash_ntz.bin 2>/dev/null || stat -f %z flash_ntz.bin) bytes)"
